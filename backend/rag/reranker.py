...
model vs. a local LLM-as-judge call), unlike the chunker/embedder, which
have exactly one plausible implementation each. Swap via constructor
...
single-user app, so the extra round-trip's latency is negligible next to
the mechanics/narrator calls already made every turn, and it avoids
...
Docker setup) for a component this app can just as well run through the
same chat client (vLLM-metal, 2026-07-13 — was Ollama before
vllm-migration-plan.md) every other LLM call in this app already depends
on anyway. CrossEncoderReranker is kept as an opt-in alternative if a
future need justifies the extra dependency weight (e.g. reranking at much
higher volume than a single local table ever will).
"""
...
class LLMJudgeReranker:
    ...
    chat model call (vLLM-metal, 2026-07-13 — was Ollama before the
    vllm-migration-plan.md migration). Slower per call than a cross-encoder,
    but no new dependency at all — reuses the same chat client every other
    LLM call in the app already needs, keeping the app container light."""
    ...
    def rerank(self, query: str, chunks: list["RuleChunk"], top_n: int) -> list["RuleChunk"]:
        ...
        llm = vllm_chat(model=self._model, base_url=self._vllm_base_url)
